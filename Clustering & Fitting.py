...
# load the dataset
df = pd.read_csv('API_AG.LND.AGRI.ZS_DS2_en_csv_v2_5447782.csv', header=2)
...
plt.title('Agricultural Land Clustering Results')
plt.xlabel('Country Index')
plt.ylabel('Agricultural Land (% of total land) in 2000')
...
plt.xlabel('Year')
plt.ylabel('Agricultural Land (% of total land)')
plt.title(f'{country} Agricultural Land Fitting')
plt.legend()
plt.show()
...
# Plot the pie chart
labels = [f'Agricultural land ({year})', 'Other land']
...
ax1.axis('equal')
plt.title(f'{country}: {value}% of land is agricultural ({year})')
plt.show()
